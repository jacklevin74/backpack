...
test('verify X1 icon is displayed (not Solana)', async () => {
  ...
  const userDataDir = path.join(__dirname, '../.playwright-user-data-icon-test');
  ...
  try {
    ...
    await page.screenshot({ path: 'e2e/screenshots/blockchain-icon-initial.png' });
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      if (src && isVisible) {
        ...
        const isSolanaIcon = src.toLowerCase().includes('solana') || src.toLowerCase().includes('sol');

        if (isX1Icon || isSolanaIcon) {
          console.log(`   Image ${i + 1}:`);
          console.log(`      src: ${src}`);
          console.log(`      alt: ${alt}`);
          console.log(`      Type: ${isX1Icon ? '✓ X1 ICON' : isSolanaIcon ? '⚠️  SOLANA ICON' : 'Unknown'}\n`);
          ...
          try {
            await img.screenshot({ path: `e2e/screenshots/blockchain-icon-${i + 1}.png` });
            console.log(`      📸 Icon screenshot saved: blockchain-icon-${i + 1}.png\n`);
          } catch (e) {
    ...
    const receiveButton = page.locator('button:has-text("Receive"), [data-testid="receive"]').first();
    ...
    console.log(`   Receive button: ${hasReceiveButton ? 'FOUND ✓' : 'Not found'}\n`);

    if (hasSendButton || hasReceiveButton) {
      // Take screenshot of the button area
      if (hasSendButton) {
        try {
          await sendButton.screenshot({ path: 'e2e/screenshots/send-button-area.png' });
...
test('suggest data-testid attributes for blockchain icons', async () => {
  ...
  console.log('   <img src="x1.png" data-testid="blockchain-icon" alt="X1" />\n');
  ...
  console.log('   await expect(page.locator("[data-testid=blockchain-name]")).toHaveText("X1");\n');
