...
test("create wallet using CDP to open extension popup", async () => {
  ...
  try {
    ...
    const hasExtensionAPI = await page.evaluate(() => {
      ...
      return {
        ...
        windowKeys: Object.keys(window)
          .filter((k) => k.includes("x1") || k.includes("wallet"))
          .slice(0, 10),
      };
    });
    ...
    console.log(
      "  window.x1:",
      hasExtensionAPI.hasX1 ? "✓ Found" : "✗ Not found"
    );
    console.log(
      "  window.solana:",
      hasExtensionAPI.hasSolana ? "✓ Found" : "✗ Not found"
    );
    console.log(
      "  window.ethereum:",
      hasExtensionAPI.hasEthereum ? "✓ Found" : "✗ Not found"
    );
    ...
    if (popupPage) {
      ...
      if (popupContent) {
        ...
        const hasCreateWallet =
          popupContent.toLowerCase().includes("create") &&
        ...
        if (hasCreateWallet) {
          ...
          const createButtons = await popupPage
            .locator(
          ...
          console.log(
            `Found ${createButtons.length} potential create button(s)`
          );
