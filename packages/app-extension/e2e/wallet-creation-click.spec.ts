...
test('click create new wallet button and follow flow', async () => {
  ...
  try {
    ...
    if (!onboardingPage) {
      console.log('⚠️  Onboarding page not auto-opened, trying to open manually...');
    ...
    await onboardingPage.screenshot({ path: 'e2e/screenshots/onboarding-initial.png' });
    ...
    if (clickableButton) {
      ...
      await clickableButton.screenshot({ path: 'e2e/screenshots/create-wallet-button.png' });
      ...
      await onboardingPage.screenshot({ path: 'e2e/screenshots/after-create-click.png' });
      console.log('📸 Screenshot: after-create-click.png\n');

      // Get new page content
      const afterClickText = await onboardingPage.locator('body').textContent();
      console.log(`📄 Page content after click: ${afterClickText?.substring(0, 300)}...\n`);

      // Look for next steps (password input, seed phrase, etc.)
      const hasPasswordInput = await onboardingPage.locator('input[type="password"]').count();
      const hasSeedPhrase = afterClickText?.toLowerCase().includes('seed') ||
                           afterClickText?.toLowerCase().includes('phrase') ||
                           afterClickText?.toLowerCase().includes('recovery');
      const hasNameInput = await onboardingPage.locator('input[type="text"], input[placeholder*="name" i]').count();
      ...
      if (hasPasswordInput > 0) {
        ...
        const allText = await onboardingPage.locator('h1, h2, h3, p, button, a').allTextContents();
      ...
      const allElements = await onboardingPage.locator('*').evaluateAll(elements =>
...
test('interact with wallet creation form fields', async () => {
  ...
  try {
    ...
    await page.screenshot({ path: 'e2e/screenshots/form-exploration.png', fullPage: true });
