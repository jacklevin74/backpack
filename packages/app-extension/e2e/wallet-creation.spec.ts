  ...
  try {
    ...
    try {
      ...
      await optionsPage.screenshot({
        path: "e2e/screenshots/options-page-initial.png",
      });
      ...
      const buttons = await optionsPage
        .locator('button, a[href*="create"], a[href*="wallet"]')
        .count();
      ...
      await optionsPage.screenshot({
        path: "e2e/screenshots/options-page-final.png",
      });
      ...
      console.log(
        "This is expected - extension pages have limitations in Playwright"
      );

      // Take screenshot of error state
      await optionsPage.screenshot({
        path: "e2e/screenshots/options-page-error.png",
      });

      // Don't fail the test - this is a known limitation
      console.log(
        "⚠️  Options page navigation blocked (expected Playwright limitation)"
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (buttonCount > 0) {
        ...
        await createButton
          .first()
          .screenshot({ path: "e2e/screenshots/create-button.png" });
      }
    } catch (error: any) {
