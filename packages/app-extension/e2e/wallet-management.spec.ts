...
test("wallet management - copy, delete, rename", async () => {
  ...
  const userDataDir = path.join(
    __dirname,
    "../.playwright-user-data-wallet-mgmt"
  );
  ...
  try {
    ...
    await page.screenshot({
      path: "e2e/screenshots/wallet-mgmt-01-initial.png",
    });
    ...
    if (copyButton) {
      ...
      await copyButton.screenshot({
        path: "e2e/screenshots/wallet-mgmt-02-copy-button.png",
      });
      ...
      console.log(
        `   🔍 Format: ${isBase58 ? "✓ Valid base58" : "⚠️  Not standard base58"}`
      );
      console.log(
        `   🔍 Full-size: ${length >= 32 ? "✓ Yes" : "⚠️  Too short"}\n`
      );
    ...
    await page.screenshot({
      path: "e2e/screenshots/wallet-mgmt-03-after-copy.png",
    });
    ...
    if (menuButton) {
      ...
      await menuButton.screenshot({
        path: "e2e/screenshots/wallet-mgmt-04-menu-button.png",
      });
      ...
      await page.screenshot({
        path: "e2e/screenshots/wallet-mgmt-05-menu-opened.png",
      });
      ...
      const hasRenameOption =
        menuText?.toLowerCase().includes("rename") ||
      ...
      const menuButtons = await page
        .locator('button, [role="menuitem"], li[role="option"]')
        .all();
      ...
      if (hasDeleteButton) {
        ...
        await page.screenshot({
          path: "e2e/screenshots/wallet-mgmt-06-delete-dialog.png",
        });
        ...
        if (hasDialog) {
          ...
          if (hasCancelButton) {
            ...
            await page.screenshot({
              path: "e2e/screenshots/wallet-mgmt-07-after-cancel.png",
            });

            // Verify dialog closed
            const afterCancelText = await page.locator("body").textContent();
            const dialogStillVisible =
              afterCancelText?.toLowerCase().includes("confirm") &&
              afterCancelText?.toLowerCase().includes("delete");

            console.log(
              `   Dialog closed: ${!dialogStillVisible ? "✓ YES" : "⚠️  Still visible"}\n`
            );
      ...
      const menuStillOpen =
        (await page
          .locator('button:has-text("Delete"), button:has-text("Rename")')
          .count()) > 0;
      ...
      await page.screenshot({
        path: "e2e/screenshots/wallet-mgmt-08-before-rename.png",
      });
      ...
      if (hasRenameButton) {
        ...
        await page.screenshot({
          path: "e2e/screenshots/wallet-mgmt-09-rename-dialog.png",
        });
        ...
        if (inputs.length > 0) {
          ...
          await page.screenshot({
            path: "e2e/screenshots/wallet-mgmt-10-name-entered.png",
          });
      ...
      await page.screenshot({
        path: "e2e/screenshots/wallet-mgmt-11-final.png",
      });
    ...
    console.log(
      `${copyButton ? "✓" : "⚠️ "} Copy button: ${copyButton ? "Found & Clicked" : "Not found"}`
    );
    console.log(
      `${menuButton ? "✓" : "⚠️ "} Menu button: ${menuButton ? "Found & Clicked" : "Not found"}`
    );
    console.log("═══════════════════════════════════════\n");
